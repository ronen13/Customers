...
function buildMailto(client) {
  ...
  const driveNote = client.googleDriveUrl ? "\nתיקיית Google Drive:\n" + client.googleDriveUrl + "\n" : "";
...
function btnStyle(variant) {
  ...
  if (variant === "red-sm")    return { ...base, background: "#fee2e2", color: "#991b1b", border: "1px solid #fca5a5", padding:"3px 10px", fontSize:12 };
  if (variant === "active-client") return { ...base, background: C.accentBg, color: C.accent, border: "1px solid #bfdbfe" };
...
function DeadlinePill({ deadline }) {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:5, borderRadius:20, padding:"3px 12px", fontSize:12, fontWeight:700,
...
function ClientCard({ client, onOpen, onDelete }) {
  ...
  return (
    <div
      ...
      style={{ ...cardStyle(), padding:"18px 22px", cursor:"pointer", display:"flex", flexDirection:"column", gap:12, position:"relative",
        boxShadow: hov ? "0 4px 18px rgba(37,99,235,.13)" : "0 1px 4px rgba(0,0,0,0.06)",
        transform: hov ? "translateY(-2px)" : "none", transition:"all .18s" }}
    >
      {/* Delete */}
      {!confirm ? (
        <button onClick={e => { e.stopPropagation(); setConfirm(true); }} title="מחק לקוח"
          style={{ position:"absolute", top:10, left:10, background:"transparent", border:"none",
            color: hov ? "#ef4444" : "#d1d5db", cursor:"pointer", fontSize:15, padding:4, borderRadius:6, lineHeight:1, transition:"color .15s" }}>✕</button>
      ) : (
        <div onClick={e => e.stopPropagation()}
          style={{ position:"absolute", top:8, left:8, background:"#fff", border:"1px solid #fca5a5", borderRadius:8,
            padding:"6px 10px", display:"flex", gap:6, alignItems:"center", boxShadow:"0 2px 8px rgba(0,0,0,0.12)", zIndex:10 }}>
          <span style={{ fontSize:12, color:"#b91c1c", fontWeight:700 }}>למחוק?</span>
          <button onClick={e => { e.stopPropagation(); onDelete(client.id); }} style={btnStyle("red-sm")}>כן</button>
          <button onClick={e => { e.stopPropagation(); setConfirm(false); }} style={{ ...btnStyle("ghost"), padding:"3px 10px", fontSize:12 }}>ביטול</button>
        </div>
      )}
...
// ── ADMIN DETAIL ──────────────────────────────────────────────────────────────
function AdminDetail({ data, setField, setChapterField, addChapter, setAddChapter, handleAddChapter, handleRemoveChapter, pct }) {
  return (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          {[["שם מלא","name","text"],["אימייל","email","email"],["טלפון","phone","text"],["שם המענק","grantName","text"],["מועד הגשה","deadline","date"],["קישור לפורטל","portalUrl","url"]].map(([label,field,type]) => (
            <label key={field} style={{ display:"flex", flexDirection:"column", gap:5 }}>
              ...
              <input type={type} value={data[field] || ""} onChange={e => setField(field, e.target.value)}
                style={{ border:"1px solid "+C.border, borderRadius:8, padding:"8px 12px", fontSize:14, outline:"none", background:"#fafafa", direction: type==="url"||type==="email"?"ltr":"rtl", fontFamily:"inherit", width:"100%", boxSizing:"border-box" }} />
            </label>
          ))}
          ...
          <label style={{ display:"flex", flexDirection:"column", gap:5, gridColumn:"1 / -1" }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <input type="url" value={data.googleDriveUrl || ""} onChange={e => setField("googleDriveUrl", e.target.value)}
              ...
              {data.googleDriveUrl && (
                <a href={data.googleDriveUrl} target="_blank" rel="noreferrer" style={{ ...btnStyle("secondary"), textDecoration:"none", whiteSpace:"nowrap" }}>📁 פתח</a>
              )}
            </div>
          </label>
        </div>
      ...
      <div style={cardStyle({ padding:"18px 24px" })}>
        ...
        <div style={{ display:"flex", gap:16, marginTop:14, flexWrap:"wrap" }}>
          {Object.entries(STATUS_CFG).map(([key,cfg]) => {
            ...
            return n > 0 ? (
              <span key={key} style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:cfg.color, fontWeight:600 }}>
                <span style={{ background:cfg.bg, border:"1px solid "+cfg.border, borderRadius:4, padding:"1px 6px" }}>{n}</span>{cfg.label}
              </span>
            ) : null;
          })}
        </div>
        ...
        <div style={{ padding:"16px 24px", borderBottom:"1px solid "+C.border, fontWeight:800, fontSize:15, display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <input value={addChapter} onChange={e => setAddChapter(e.target.value)} onKeyDown={e => e.key==="Enter" && handleAddChapter()}
              placeholder="הוסף פרק חדש..."
              style={{ border:"1px solid "+C.border, borderRadius:8, padding:"6px 12px", fontSize:13, outline:"none", direction:"rtl", width:180, fontFamily:"inherit" }} />
            <button onClick={handleAddChapter} style={btnStyle("secondary")}>+ הוסף</button>
          </div>
        </div>
        {Object.entries(data.chapters).map(([ch,val],i,arr) => (
          <div key={ch} style={{ display:"grid", gridTemplateColumns:"1fr 150px 1fr auto", alignItems:"center", gap:14, padding:"14px 24px",
            ...
            <select value={val.status} onChange={e => setChapterField(ch,"status",e.target.value)}
              style={{ border:"1px solid "+STATUS_CFG[val.status].border, background:STATUS_CFG[val.status].bg, color:STATUS_CFG[val.status].color,
                borderRadius:8, padding:"6px 10px", fontSize:13, fontWeight:700, outline:"none", cursor:"pointer", direction:"rtl", fontFamily:"inherit" }}>
              {Object.entries(STATUS_CFG).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
            <input placeholder={val.status==="missing"?"פרט מסמכים חסרים...":"הערה (אופציונלי)"} value={val.note}
              onChange={e => setChapterField(ch,"note",e.target.value)}
            ...
            <button onClick={() => handleRemoveChapter(ch)}
              style={{ background:"transparent", border:"none", color:"#cbd5e1", cursor:"pointer", fontSize:20, padding:"0 4px", lineHeight:1 }}>×</button>
          </div>
        ))}
      </div>
    </div>
...
function ClientFacing({ data, pct, missing }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ background:"linear-gradient(135deg, #0f2744 0%, #1e4db7 100%)", borderRadius:16, padding:"28px 32px", color:"#fff" }}>
        ...
        <div style={{ marginTop:22, display:"flex", gap:28, flexWrap:"wrap" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:16, fontWeight:700 }}>{fmtDate(data.deadline)}</div>
      ...
      {missing.length > 0 && (
        <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:14, padding:"18px 24px" }}>
          ...
          <div style={{ display:"flex", gap:10, marginTop:14, flexWrap:"wrap" }}>
            {data.portalUrl && <a href={data.portalUrl} target="_blank" rel="noreferrer" style={{ ...btnStyle("danger"), textDecoration:"none" }}>📎 העלאת מסמכים לפורטל</a>}
            {data.googleDriveUrl && <a href={data.googleDriveUrl} target="_blank" rel="noreferrer" style={{ ...btnStyle("secondary"), textDecoration:"none" }}>📁 פתח Google Drive</a>}
          </div>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:16, justifyContent:"center", flexWrap:"wrap" }}>
        {data.portalUrl && <a href={data.portalUrl} target="_blank" rel="noreferrer" style={{ color:C.accent, fontSize:13, fontWeight:600, textDecoration:"none" }}>🔗 כניסה לפורטל האישי</a>}
        {data.googleDriveUrl && <a href={data.googleDriveUrl} target="_blank" rel="noreferrer" style={{ color:"#15803d", fontSize:13, fontWeight:600, textDecoration:"none" }}>📁 תיקיית Google Drive</a>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Heebo','Assistant',sans-serif", direction:"rtl", color:C.text }}>
      <div style={{ background:C.navy, color:"#fff", padding:"0 32px", display:"flex", alignItems:"center", justifyContent:"space-between", height:60, boxShadow:"0 2px 8px rgba(0,0,0,0.18)" }}>
        <div>
          <div style={{ fontSize:18, fontWeight:800, letterSpacing:-0.5 }}>מערכת ניהול מענקים</div>
          <div style={{ fontSize:12, opacity:0.6 }}>פורטל ניהול תיקים</div>
        </div>
        <button onClick={onBack} style={{ ...btnStyle("ghost"), color:"#fff", border:"1px solid rgba(255,255,255,0.3)" }}>← חזרה</button>
      </div>
            ...
            <div style={{ display:"flex", background:"#e2e8f0", borderRadius:8, padding:3 }}>
              <button
                ...
                style={{ padding:"6px 14px", borderRadius:6, border:"none", cursor:"pointer", fontFamily:"inherit",
                  ...
                  background: view==="admin" ? C.accent : "transparent",
                  color: view==="admin" ? "#fff" : C.muted }}
              >🗂 ניהול</button>
              <button
                onClick={() => setView("client")}
                style={{ padding:"6px 14px", borderRadius:6, border:"none", cursor:"pointer", fontFamily:"inherit",
                  fontWeight:700, fontSize:13, transition:"all .15s",
                  background: view==="client" ? C.accentBg : "transparent",
                  color: view==="client" ? C.accent : C.muted }}
              >👁 תצוגת לקוח</button>
            </div>
            {missing.length > 0 && (
              <a href={buildMailto(data)} style={{ ...btnStyle("danger"), textDecoration:"none" }}>
                ✉ מייל חסרים ({missing.length})
              </a>
            )}
            {data.googleDriveUrl && (
              <a href={data.googleDriveUrl} target="_blank" rel="noreferrer" style={{ ...btnStyle("secondary"), textDecoration:"none" }}>📁 Drive</a>
            )}
...
export default function App() {
  ...
  function deleteClient(id) { setClients(cs => cs.filter(c => c.id !== id)); }
  function addClient() {
    const id = Date.now();
    const nc = {
      id, name: "לקוח חדש", email: "", phone: "", grantName: "שם המענק",
      deadline: new Date(Date.now() + 30*86400000).toISOString().slice(0,10),
      portalUrl: "", googleDriveUrl: "",
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Heebo','Assistant',sans-serif", direction:"rtl", color:C.text }}>
      <div style={{ background:C.navy, color:"#fff", padding:"0 32px", display:"flex", alignItems:"center", justifyContent:"space-between", height:60, boxShadow:"0 2px 8px rgba(0,0,0,0.18)" }}>
        ...
        <button onClick={addClient} style={btnStyle("primary")}>+ לקוח חדש</button>
      </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"28px 20px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))", gap:14 }}>
          {filtered.map(c => <ClientCard key={c.id} client={c} onOpen={() => setSelected(c)} onDelete={deleteClient} />)}
